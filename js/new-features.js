...
// Biggest Movers · Altcoin Season · BTC Dominance
...
// Get coins — use already-loaded ALL_COINS first (no extra API call needed)
function getCoins() {
  if (typeof ALL_COINS !== 'undefined' && ALL_COINS.length > 5) return ALL_COINS;
  return [];
}
...
async function loadBiggestMovers() {
  ...
  try {
    // Use ALL_COINS already in memory — no extra API call needed
    let coins = getCoins();
    if (!coins.length) {
      el.innerHTML = '<div style="text-align:center;padding:40px;font-family:Space Mono,monospace;font-size:11px;color:var(--muted)">⟳ Крипто үнийн мэдээлэл ачаалж байна…<br><br><button onclick="loadBiggestMovers()" style="margin-top:10px;padding:8px 18px;background:var(--accent);border:none;border-radius:4px;font-family:Space Mono,monospace;font-size:10px;color:#000;cursor:pointer;font-weight:700">↻ Дахин оролдох</button></div>';
      return;
    }
...
async function loadAltcoinSeason() {
  ...
  try {
    ...
    let coins = getCoins();
    if (!coins.length) {
      el.innerHTML = '<div style="text-align:center;padding:40px;font-family:Space Mono,monospace;font-size:11px;color:var(--muted)">⟳ Мэдээлэл ачаалж байна…</div>';
      return;
    }

    // Use 7d change as proxy for 90d (we only have 7d in ALL_COINS)
    const btc = coins.find(c => c.id === 'bitcoin');
    const btc7d = btc?.price_change_percentage_7d_in_currency || btc?.price_change_percentage_24h || 0;

    const alts = coins.filter(c => c.id !== 'bitcoin');
    const outperformed = alts.filter(c => {
      const chg = c.price_change_percentage_7d_in_currency || c.price_change_percentage_24h || 0;
      return chg > btc7d;
    }).length;
    const score = Math.round((outperformed / (alts.length || 1)) * 100);
    ...
    el.innerHTML = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:20px">'
      ...
      + 'BTC 7d: <span style="color:#f4c542">' + nfPct(btc7d) + '</span><br>'
      ...
      + '<div style="font-family:Space Mono,monospace;font-size:9px;letter-spacing:2px;color:var(--muted);text-transform:uppercase;margin-bottom:14px">Top Outperformers vs BTC (7d)</div>'
      ...
      + '<th style="font-family:Space Mono,monospace;font-size:9px;color:var(--muted);padding:8px 10px;text-align:right">7d Return</th>'
      + '<th style="font-family:Space Mono,monospace;font-size:9px;color:var(--muted);padding:8px 10px;text-align:right">vs BTC</th>'
      + '</tr></thead><tbody>'
      + alts.sort((a,b) => (b.price_change_percentage_7d_in_currency||a.price_change_percentage_24h||0) - (b.price_change_percentage_7d_in_currency||b.price_change_percentage_24h||0))
        .slice(0, 10).map(function(c) {
          const ret = c.price_change_percentage_7d_in_currency || c.price_change_percentage_24h || 0;
          const vsBtc = ret - btc7d;
...
async function loadBtcDominance() {
  ...
  try {
    // Try to use already-loaded global data first
    let globalData = null;
    if (typeof CR_GLOBAL !== 'undefined' && CR_GLOBAL?.data?.market_cap_percentage) {
      globalData = CR_GLOBAL;
    } else {
      globalData = await nfFetch(NF_API + '/api/crypto/global', NF_CG + '/global');
    }
